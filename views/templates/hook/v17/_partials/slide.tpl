...
{if $element}
    <li {if $element->html_id}id="{$element->html_id}"{/if} class="mdghome-slide splide__slide {$element->html_class}">
        ...
        {if $element->display_title || $element->display_text}
            <div class="mdghome-slide-caption" {if $element->color_bg || $element->color_text}style="{if $element->color_bg}background-color: {$element->color_bg};{/if}{if $element->color_text}color: {$element->color_text}{/if}"{/if}>
